...
- Directory tree:
  ...
  ├── alembic
  │   └── versions
  │       ├── ab13826fb264_create_initial_tables.py
  │       ├── f51adb7c4d0d_add_task_complete_columns.py
  ├── alembic.ini
  ├── do_it_2.py
  ├── do_it_3.py
  ├── do_it.py
  ...
  │   └── spiders
  │       ├── quotes_spider.py
  │       ├── selenium_quotes_spider.py
  │       └── vocabulary_spider.py
  ├── gre_words.sql
  ├── init.py
  ```
- The _alembic_ subdirectory is mostly defaults generated by:
  ```
  $ alembic init alembic
  ```
  However, the _versions_ subdirectory contains two database migrations that I wrote.

- _alembic.ini_ has one change from the default: I point `sqlalchemy.url` to a local SQLite database `sqlite:///gre_words.sqlite`.

- _gre_words.sqlite_: This is the current database. Not under revision control. I may however place an SQL dump under revision control, if it isn't too big.

- _gre_words/settings.py_: This is modified from the default in two ways:
  - I've changed it to use Scrapy-Selenium to download html.
  - I've told it to a Chrome browser to download html, under Selenium control.
  - I've told Chrome to use as data directory _/mnt/Work/Repos/irrealis/flashcards/spiders/google-chrome-config_.
    - Once Chrome was launched, I manually setup my _irrealis_ Chrome profile, and also logged into my vocabulary.com account. This info now persists between launches of Chrome under Scrapy-Selenium.

- _gre_words/db_interface.py_: This contains the same db code as what's in this notebook page, minus the table definitions and the sample code.

- _gre_words/spiders/*.py_: Three fledgeling Scrapy spiders. I'm not using these yet, so the code hasn't been developed.

- The following files just contain sample code that I'm playing with:
  - _init.py_: I don't use this anymore. It contains sample code for setting up the database interface.

  - _do_it.py_: Code, run in a Scrapy shell, to launch a Chrome browser under Selenium control, and use it to:
    - Visit https://www.vocabulary.com.
    - Look at the vocabulary lists I've created.
    - Navigate to the first vocabulary list.
    - Extract info about the first word in the list.

  - _do_it_2.py_: Code, run in a scrapy shell, to:
    - Setup the db interface.
    - Create a vocabulary list db object.
    - Populate the vocabulary list db object with words from the first vocabulary list.
    - This file also has commented-out code that plays around with database interface, doing things like:
      - Create a vocab list object.
      - Create a word object, adding it to the vocab list object.
      - Create a word meaning sense object, adding the word object to the sense object.
      - Create a related sense object, adding the relationship of kind `type_of`, adding some word objects to the related sense object.

  - _do_it_3.py_: Contains code, some of it commented out, to:
    - Navigate to https://www.vocabulary.com/account/lists to look at my vocab lists.
    - Get info about the fourth vocab list: its ID, description, and relative URL.
    - Create a corresponding vocab list object in the db.
    - Sleep for a few seconds, then navigate to the vocab list page.
    - Scrape the words in the vocab list, adding them to the database, associated with the vocab list object.
      - This has some code to recover from an error I noticed, when a word's html describes the word's frequency using a unicode "infinity" symbol.
